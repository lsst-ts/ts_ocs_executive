      ...
      if ( status == SAL_atcs.SAL__OK ) {
        ...
        try {
          Executive.getEntityMap()
              .get( EntityType.ATCS.toSubString() )._modelStateTransitionQ.put( event.summaryState );

          Executive.getEntityMap()
              .get( EntityType.ATCS.toSubString() )._viewStateTransitionQ.put( event.summaryState );
